    ...
    out_file = "aggregated.nc")
{
  ...
  space_dims <- dims_all[grepl("lat|lon", dims_all, ignore.case = TRUE)]
  ...
  # Validate time shift
  if (!is.numeric(time_shift) || length(time_shift) != 1) {
    stop("`time_shift` must be a single numeric value (in hours).")
  }
  
  # Validate output file name
  if (!is.character(out_file) || length(out_file) != 1) {
    stop("`out_file` must be a character string.")
  }
  ...
  space_dims <- names(dims)[sapply(dims, function(d) grepl("degree", d$units, ignore.case = TRUE))]
  ...
  for (d in space_dims)
  {
    dim_defs[[d]] <- ncdim_def(d, dims[[d]]$units, ncvar_get(nc, d))
  }
  ...
  out_nc <- nc_create(out_file, var_defs)
  for (i in seq_along(var))
  {
    ...
    for(t in 1:length(times_id_grouped))
    {
      ...
      vals<-apply(ncvar_get(nc,var[i],start = start,count = count),1:2,fun[[var[i]]])*aggregationFactor[i]
      count[time_dim_id]<-1
      ncvar_put(nc = out_nc,varid = varid,vals = vals,start = c(1,1,t),count = count)
      setTxtProgressBar(pb_time, t)  # Update inner progress bar
    }
    close(pb_time)
  }
  ncatt_put(out_nc, 0, "title", "Aggregated NetCDF File")
  ncatt_put(out_nc, 0, "institution", "University of Waterloo")
  ncatt_put(out_nc, 0, "source", "Canadian Surface Reanalysis (CaSR v3.1) from Environment and Climate Change Canada (ECCC)")
  ncatt_put(out_nc, 0, "contact", "rarabzad@uwaterloo.ca")
  ncatt_put(out_nc, 0, "location", "Waterloo, Canada")
  ncatt_put(out_nc, 0, "history", paste("Created on", Sys.time()))
  ncatt_put(out_nc, 0, "Conventions", "CF-1.6")
  nc_close(nc)
  nc_close(out_nc)
  message("✅ Aggregated NetCDF written to: ", out_file)
}
